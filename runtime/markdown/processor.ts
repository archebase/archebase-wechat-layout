...
export type RemarkPlugin = Plugin<[], any>;
...
export function normalizeContainerSyntax(md: string): string {
  // Remove space after opening :::name.
  // Use [ \t] (not \s) to stay on the same line — \s matches \n and would
  // cross line boundaries, gobbling up blank lines and merging adjacent text.
  return md.replace(/^:::[ \t]+([^\s:]+)/gm, ':::$1');
}

export function createProcessor(mathRenderer?: MathRenderer, remarkPlugins: RemarkPlugin[] = []) {
  const p = unified()
    .use(remarkParse)
    .use(remarkGfm)
    .use(remarkDirective)
...
export async function processMarkdownWithMath(
  md: string,
  mathRenderer: MathRenderer,
  remarkPlugins: RemarkPlugin[] = [],
): Promise<string> {
  const normalized = normalizeContainerSyntax(md);
  const result = await createProcessor(mathRenderer, remarkPlugins).process(normalized);
  return String(result);
}
